...
def norm_01(score_list):
    ...
    mini = min([i for i in score_list if i is not None])
    maxi = max([i for i in score_list if i is not None])
    ...
    for i in range(len(fc1)):
        if r[i] != 0.0:
            if fc1[i] == 0.0:
                ...
                d += (fc1[i] * math.log(fc1[i]/r[i], 2) + fc2[i] * math.log(fc2[i]/r[i], 2))
...
for opt, arg in opts:
    if opt == "-h":
        ...
        try:
            ...
            print("Using a gap cutoff of %.1f...\n" % column_gap_cutoff, file=sys.stderr)
...
print("Scored %s with %s matrix, window length=%d, and lambda=%.2f.\n" \
    % (align_file, matrix_file, cons_win_len, lamb), file=sys.stderr)
